...
| MiniMax-H3 | Packed varlen FA (NTD + cu_seqlens) on Ascend NPU | Experimental | Runs per-block FP8 per document via the MindIE-SD FIA operator; requires an FP8-capable NPU (A5 class). `MINDIESD_SET_FREQ_MANUAL=true` caps the AI-core frequency to 1400 MHz around each FIA call and restores 1650 MHz after |
