...
class FlashAttentionImpl(AttentionImpl[AttentionMetadata]):
    ...
    def __init__(
        ...
        **extra_impl_args,
        ...
        # Lazily created Ascend causal masks for npu_fusion_attention
        # sparse_mode 2/3, keyed by device. See _get_npu_causal_mask.
        self._npu_causal_masks: dict = {}
    ...
    # ── Ascend piecewise (block-causal) support ──
    #
    # sparse_mode of torch_npu.npu_fusion_attention:
    ...
    #                        when Sq != Skv.
    # No sparse_mode applies causal masking internally without a mask (with
    # atten_mask=None the sparse_mode parameter is ignored entirely), so modes
    # 2/3 take the fixed compressed lower-triangular mask at exactly
    # (2048, 2048); the kernel tiles it for longer sequences.
    _NPU_SPARSE_LEFT_UP_CAUSAL = 2
    _NPU_SPARSE_RIGHT_DOWN_CAUSAL = 3
    _NPU_CAUSAL_MASK_SIZE = 2048

    def _get_npu_causal_mask(self, device: torch.device) -> torch.Tensor:
        """Return the cached (2048, 2048) causal mask for sparse_mode 2/3.
        ...
        triangle.
        """
        mask = self._npu_causal_masks.get(device)
        if mask is None:
            mask = torch.triu(
                torch.ones(
                    self._NPU_CAUSAL_MASK_SIZE,
                    self._NPU_CAUSAL_MASK_SIZE,
                    dtype=torch.bool,
                    device=device,
                ),
            ...
            self._npu_causal_masks[device] = mask
        return mask

    def _npu_piecewise_attn_func(
        self,
        query: torch.Tensor,
        key: torch.Tensor,
        value: torch.Tensor,
    ...
    def _forward_piecewise_npu(
        ...
        logger.debug("Using piecewise NPU attention for mixed causal/full mask")
        return piecewise_attn(
            query,
            key,
            value,
