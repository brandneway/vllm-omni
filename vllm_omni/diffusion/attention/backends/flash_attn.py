...
class FlashAttentionImpl(AttentionImpl):
    ...
    def forward_npu(
        ...
        out = self._forward_prefix_kv_slice_quant_npu(
            query,
            key,
            value,
            extra,
    ...
    def _resolve_packed_seq_npu(
        ...
        With AllGather-KV sequence parallelism (optionally composed with
        Ulysses), the kernel sees local Q against global K/V. When
        ``query_ranges`` carries the single packed range produced by the
        AllGather-KV strategy, the local Q rows are a contiguous slice of the
        same global real document, so the [real, pad] contract generalizes:
        seq_q becomes that slice's [local_real, local_total] pair while seq_k
        keeps the global [real, pad] pair.
        """
        if self.causal:
            return None
        packed_keys = ("cu_seqlens_q", "cu_seqlens_k", "max_seqlen_q", "max_seqlen_k")
